...
public class PriorityQueue extends AbstractQueue
    ...
    private void fixUp(int k) {
        if (comparator == null) {
            ...
            while (k > 1) {
                int parent = k >>> 1; //parentNo = (nodeNo-1)/2
                if (comparator.compare((Object)queue[parent], (Object)queue[k]) <= 0)
                    break;
                Object tmp = queue[parent];  queue[parent] = queue[k]; queue[k] = tmp;
                k = parent;
            }
        }
    }
     ...
     * rooted at parentNo, which is assumed to satisfy the heap invariant except
     * possibly for node parentNo itself (which may be greater than its children).
     *
     * This method functions by "demoting" queue[parentNo] down the hierarchy
     * (by swapping it with its smaller child) repeatedly until queue[parentNo]
     * is less than or equal to its children.
     */
    private void fixDown(int parentNo) {
        int child;
        if (comparator == null) {
            while ((child = parentNo << 1) <= size && (child > 0)) {
                if (child<size &&
                    ((Comparable)queue[child]).compareTo((Object)queue[child+1]) > 0)
                    child++; // child indexes smallest kid

                if (((Comparable)queue[parentNo]).compareTo((Object)queue[child]) <= 0)
                    break;
                Object tmp = queue[child];  queue[child] = queue[parentNo]; queue[parentNo] = tmp;
                parentNo = child;
            }
        } else {
            while ((child = parentNo << 1) <= size && (child > 0)) {
                if (child<size &&
                    comparator.compare((Object)queue[child], (Object)queue[child+1]) > 0)
                    child++; // child indexes smallest kid
                if (comparator.compare((Object)queue[parentNo], (Object)queue[child]) <= 0)
                    break;
                Object tmp = queue[child];  queue[child] = queue[parentNo]; queue[parentNo] = tmp;
                parentNo = child;
            }
